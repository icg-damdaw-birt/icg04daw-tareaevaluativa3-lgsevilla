...
describe('Movies Store (Svelte 5 Runes)', () => {
  ...
  describe('toggleFavorite()', () => {
    it('debería cambiar el estado favorito de una película', async () => {
      vi.mocked(api.getMovies).mockResolvedValue([...mockMovies]);
      await moviesStore.loadMovies();
    ...
    it('debería desfavoritar una película que ya es favorita', async () => {
      const favoriteMovie: Movie = { ...mockMovies[0], isFavorite: true };
      vi.mocked(api.getMovies).mockResolvedValue([favoriteMovie, ...mockMovies.slice(1)]);
      await moviesStore.loadMovies();

      const unfavoriteMovie: Movie = { ...favoriteMovie, isFavorite: false };
      vi.mocked(api.toggleFavorite).mockResolvedValue(unfavoriteMovie);
    ...
    it('debería manejar error al cambiar favorito', async () => {
      vi.mocked(api.toggleFavorite).mockRejectedValue(new Error('Server error'));
